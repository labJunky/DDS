...
namespace DDSControl
{
    public class AD9958MessageFactory
    {
        ...
        public Message SetFallingRampRate(double FallingRampRate)
        {
            Message msg = new Message();
            msg.Add(registerByShortName["LSRR"].Address);
            msg.Add(calculateRampRateWord(FallingRampRate));
            if (log.IsDebugEnabled) { log.DebugFormat("Generated message to set falling ramp rate to {1}: {2}", FallingRampRate, msg); }
            return msg;
        }
        ...
        public Message SetDeltaFrequency(double RisingDeltaFrequency, double FallingDeltaFrequency)
        {
            Message msg = new Message();
            msg.Add(SetRisingDeltaFrequency(RisingDeltaFrequency));
            msg.Add(SetFallingDeltaFrequency(FallingDeltaFrequency));
            return msg;
        }
